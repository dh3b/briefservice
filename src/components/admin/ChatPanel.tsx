            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {chatMessages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender_type === "admin" ? "justify-end" : "justify-start"}`}>
                  <div className={`flex flex-col gap-0.5 max-w-[70%] ${msg.sender_type === "admin" ? "items-end" : "items-start"}`}>
                    <div
                      className={`w-fit px-4 py-2.5 rounded-2xl text-sm break-words ${msg.sender_type === "admin" ? "bg-primary text-primary-foreground rounded-br-md" : "bg-secondary text-secondary-foreground rounded-bl-md"}`}
